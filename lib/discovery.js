...
var Emitter = require('node:events');
...
class Discovery extends Emitter {
    ...
    search(options, callback) {
        ...
        for (var [, iface] of this.interfaces) {
            ...
            headers.host = iface.family === 'IPv6' ? `[${host}]:${SSDP.PORT}` : `${host}:${SSDP.PORT}`;
    ...
    listen(interfaces, callback) {
        ...
        this.interfaces.forEach((iface, _address) => {
            var socket = iface.socket;

            socket.on('error', error => {
                this.emit('warning', error, socket);
            });
            socket.on('message', (msg, rinfo) => {
                this._handleMessage(msg, rinfo, iface);
            });
            socket.on('listening', () => {
            ...
            socket.bind(SSDP.PORT, () => {
                ...
                if ((socketsBound += 1) === interfaceCount) {
                    var error = !this.interfaces.size ? new Error('No interfaces bound') : null;
