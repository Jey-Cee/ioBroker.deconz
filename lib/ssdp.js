...
SSDP.parseResponse = function (buffer) {
    ...
    for (var i = 0; i < header.length; i++) {
        ...
        message.headers[k] = message.headers[k] != null ? `${message.headers[k]}, ${v}` : v;
    }
...
SSDP.parseRequest = function (buffer) {
    ...
    var [method, path, protocol] = header.length ? header.shift().split(/\s/g) : [];
