...
class deconz extends utils.Adapter {
    ...
    async onReady() {
        adapter = this; // eslint-disable-line @typescript-eslint/no-this-alias
        await main();
    }
                ...
                switch (dp) {
                    case 'bri':
                        if (state.val > 0 && (transitionTime === 'none' || transitionTime === 0)) {
                            ...
                            parameters = `{"transitiontime": ${JSON.stringify(transitionTime)}, "bri": ${JSON.stringify(
                                state.val,
                            )}, "on": true}`;
                        } else {
                            parameters = `{"bri": ${JSON.stringify(state.val)}, "on": false}`;
                        }
                        await SetObjectAndState(tmp[3], '', tmp[2], 'level', Math.round((100 / 255) * state.val));
                        break;
                    case 'level':
                        if (state.val > 0 && (transitionTime === 'none' || transitionTime === 0)) {
                            parameters = `{"bri": ${Math.round((255 / 100) * state.val)}, "on": true}`;
                        } else if (state.val > 0) {
                            parameters = `{"transitiontime": ${JSON.stringify(transitionTime)}, "bri": ${Math.round(
                                (255 / 100) * state.val,
                            )}, "on": true}`;
                        } else {
                    ...
                    case 'hue':
                        if (transitionTime === 'none' || transitionTime === 0) {
                            ...
                            parameters = `{"transitiontime": ${JSON.stringify(transitionTime)}, "hue": ${Math.round(
                                parseInt(JSON.stringify(state.val)) * hue_factor,
                            )}}`;
                        }
                        break;
                    case 'sat':
                        if (transitionTime === 'none' || transitionTime === 0) {
                            ...
                            parameters = `{"transitiontime": ${JSON.stringify(transitionTime)}, "sat": ${JSON.stringify(
                                state.val,
                            )}}`;
                        }
                        break;
                    case 'ct':
                        if (transitionTime === 'none' || transitionTime === 0) {
                            ...
                            parameters = `{"transitiontime": ${JSON.stringify(transitionTime)}, "ct": ${JSON.stringify(
                                state.val,
                            )}}`;
                        }
                        break;
                    ...
                    case 'alert':
                        if (transitionTime === 'none' || transitionTime === 0) {
                            ...
                            parameters = `{"transitiontime": ${JSON.stringify(
                                transitionTime,
                            )}, "alert": ${JSON.stringify(state.val)}}`;
                        }
                        break;
                    case 'colorspeed': {
                    ...
                    case 'effect': {
                        if (state.val === 'colorloop') {
                            ...
                            parameters = `{"effect": ${JSON.stringify(state.val)}, "effectSpeed":  ${JSON.stringify(effectspeed)}, "effectColours": ${JSON.stringify(
                                effectcolours
                                    ? effectcolours.val
                                    : [
                                          [255, 0, 0],
    ...
    async onMessage(obj) {
        ...
        if (obj) {
            switch (obj.command) {
                ...
                case 'deleteAPIkey':
                    await deleteAPIkey();
                    this.sendTo(obj.from, obj.command, { native: { user: '' } }, obj.callback);
                    wait = true;
                    break;
                case 'getConfig':
                    await getConfig();
                ...
                case 'deleteLight':
                    await deleteLight(obj.message, res => {
                        if (obj.callback) {
                            this.sendTo(obj.from, obj.command, JSON.stringify(res), obj.callback);
                        }
                    });
                    wait = true;
                    break;
...
function autoDiscovery() {
    ...
    discovery.on('message', (msg, rinfo, _iface) => {
...
function heartbeat() {
    discovery.on('notify', (msg, _rinfo, _iface) => {
...
async function createLightDevice(list, keyName, lightID) {
    if (list[keyName]['state']) {
        ...
        for (let z = 0; z <= count2; z++) {
            ...
            if (
                list[keyName]['type'] !== 'On/Off plug-in unit' &&
                list[keyName]['type'] !== 'On/Off light' &&
                list[keyName]['type'] !== 'Configuration tool'
            ) {
 ...
 * @param {string} _type - first letter has to be upper case. Possible: Lights, Sensors
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function getDeviceByID(_deviceID, _type) {}
 ...
 *
 * @returns {Promise<object>} A promise that resolves to an object containing the gateway parameters.
...
async function deleteDevice(deviceId) {
    await adapter.getObjectListAsync({ startkey: deviceId, endkey: `${deviceId}.\u9999` }).then(
        async result => {
...
async function SetObjectAndState(id, name, type, stateName, value) {
    ...
    switch (stateName) {
        ...
        case 'bri': {
            ...
            objMax = 255;
            objDefault = 254;
            await SetObjectAndState(id, name, type, 'level', Math.round((100 / 255) * value));
            break;
        }
        case 'buttonevent':
            objType = 'number';
        ...
        case 'lux':
            objType = 'number';
            ((objRole = 'value.brightness'), (objWrite = false));
            objDefault = 0;
            objUnit = 'Lux';
            break;
        case 'measured_value':
